...
# wget http://php.net/get/php-5.5.11.tar.gz/from/this/mirror -O php-5.5.11.tar.gz
# tar xvf php-5.5.11.tar.gz && cd php-5.5.11
```
...
- Make and build deb package
	...
	```sh
	# make
	# echo "PHP 5.5.11" > description-pak && checkinstall -Dy --install=no --nodoc make -i install
	```

- PHP deb package has now been created with the following filename:
	- `php_5.5.11-1_amd64.deb` for **64 bit** Ubuntu server
	- `php_5.5.11-1_i386.deb` for **32 bit** Ubuntu server
- Review then install the built deb package, optionally save `php_5.5.11-1_*.deb` package for later use (recommended)

	```sh
	# dpkg -c php_5.5.11-1_*.deb
	# dpkg -i php_5.5.11-1_*.deb
	```
